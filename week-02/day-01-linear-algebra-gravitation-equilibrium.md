...
$$A= \left[\,[1,2],[3,4]\,\right], \qquad \vec{x}= \left(x,y\right)^T$$

Then:

$$A\vec{x} = \left(x+2y,3x+4y\right)^T$$

Column view:

$$A\vec{x} = x \left(1,3\right)^T + y \left(2,4\right)^T$$
...
$$\left[\,[1,2],[3,4]\,\right] \left(x,y\right)^T = \left(5,11\right)^T$$
...
$$\text{span}(\vec v_1,\ldots,\vec v_n) = \{c_1\vec v_1+\cdots+c_n\vec v_n\}$$
...
$$\vec e_1= \left(1,0\right)^T, \quad \vec e_2= \left(0,1\right)^T$$
...
$$\left(2,4\right)^T = 2 \left(1,2\right)^T$$
...
$$\vec v= \left(3,5\right)^T = 3 \left(1,0\right)^T + 5 \left(0,1\right)^T$$
...
$$\vec v_1= \left(1,1,0\right)^T, \quad \vec v_2= \left(2,0,1\right)^T, \quad \vec v_3= \left(3,1,1\right)^T$$
...
$$\left(1,2\right)^T, \quad \left(a,4\right)^T$$

They are dependent when:

$$\det\left(\left[\,[1,a],[2,4]\,\right]\right)=0$$
...
$$\boxed{ K=\frac{a_C^c a_D^d}{a_A^a a_B^b} }$$
...
$$\det\left(\left[\,[a,b],[c,d]\,\right]\right)=0$$
